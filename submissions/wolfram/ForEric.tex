...
\begin{abstract}
We propose and investigate a model for marriage based on a stochastic matching process and simple behavioral rules. Agents have preferences that are determined by a probability distribution. They search for better mates at random, forming new couples and splitting up in the process. Marriage corresponds to the behavior of stopping searching for a better mate when the affinity between a couple exceeds a certain value. We show that the average utility in the system with marriage can be higher than in the system without it. To test the adequacy of our model to describe real societies, we compare the evolution of the fraction of married couples to real-world data and obtain good agreement. In the last section, we formulate the model in the limit of an infinite number of agents and find an analytical expression for the evolution of the system.
\end{abstract}
...
%The model we propose will enable us to give a precise answer to this question. %To make these ideas precise and answer this question, we propose a search model for courtship and marriage.
%The answer to this question depends on how much you like your mate, as can be seen by considering the two extremes. Suppose you are with the love of your life, i.e., the person you like more among everyone in society. In this case, the probability of meeting a better mate is zero, and therefore it will be beneficial to get married. In the other extreme, suppose you are indifferent between being with your mate and being single. In this case, being left will be outweighed by the potential benefit of meeting a person you like more and therefore you should not propose marriage. Out of the extremes, the answer is not straightforward, and so we are led to the question: how much should we like our mate to propose marriage? Or put simply in a slightly abuse of language: who should we marry? To answer this question and to make these ideas precise, we will propose a search model for the matching of couples in large societies.

A famous problem related to this subject is the stable matching problem \cite{10.2307/2312726}. It is the problem of finding a stable matching between two groups of agents with preferences among themselves. If we interpret the groups as man and woman, then a matching is a bijection between these two genders, and a matching is stable if there are no man and woman who both prefer each other over their respective mate. Since its inception, there has been extensive research on the mathematical structure of stable matching and related algorithmic questions \cite{10.2307/2312726,knuthstable,gusfield1989stable,roth1992two}. The notion of stable matching is fundamental to understand a variety of markets, as one can find in the recent review \cite{ren2021matching}. Furthermore, matching theory is a relevant tool for market design \cite{article,NBERw29367}. However, when it comes to large societies, the concept of stable matching may not be important. After all, it is likely that there are two agents that would rather be together than be with their respective mates. But, in a large society, this blocking pair may not yet know each other. Also, the likelihood that this will happen depends on the type of relationship they have. If they are married, in principle, they will not be open to this possibility. Otherwise, they may eventually match a better mate. To make these ideas precise and to answer the question from the previous paragraph, we propose a model for dating and marriage based on a random matching process and simple behavioral rules.
...
%The ubiquitous emergence of complex patterns from simple rules is clearly presented and argued for in \cite{Wolfram02anew}.

%The model also challanges the paradigm of utility maximization in a particular way. The interaction between agents in the maching market gives rise to a complex partern that agents can't forcast. Their behavior is modulated by a idiosincratic parameter. This parameter, could, in principle, be maximized in order to give the maximal utility. However, such computation hard and agents can't forcast. One may phrase this as a sympton of computational irreducibility. The existence of such complex structure in systems of agents that interact in some particular way, gives a new opportunity for economists. By modeling such systems, they are able to forcast its evolution and advice actual societies on the proper way to behave. Computational irreducibility gives a new avenue to lift economics from a descriptive science to a enunciative one.  

%Many times in history, new generations challenge traditional institutions as they see them as obsolete. Marriage is one of those challenged institutions. In this context, especially the younger generations, look for new ways to relate affectionately to each other. %As part of this generation, I participate in different kinds of relationships, in particular, open relationships.
%Open relationships are one of those attempts. Roughly, in an open relationship, the two participants do not abdicate from relating with others. There are many types of open relationships: in some, the participants agree that they can match with others, but they can't get involved. In others, the participants can match with new mates but just once. A diversity of different setups are been tried, but for a significant fraction of those attempts, the participants do not abdicate from matching with other mates. As a consequence, one is at higher risk of falling in love with someone else, thus breaking apart and starting a new relation. In contrast, one of the prominent features of marriage is this abdication. When two agents get marriage, in principle, part of the agreement is that they will stop searching for new mates, and thus the relation will not end because one of the participants found a better mate.

%Before thinking more seriously on this subject, 
%Naively, an open relationship may be seen as better than marriage. After all, the difference between an open relation and marriage is that in the latter you abdicate something. And at the same time, if one has fewer possibilities one is worst off. Therefore, an open relationship would be better than marriage. However, this is not the complete history. In an open relationship, you indeed have more possibilities, but one of these possibilities is to be left because your mate found someone better. One is then lead to the question: does the odds of finding someone better pays the cons of being left out by one's mate. The model proposed in this paper, was our attempt to investigate this question. But also has several different interpretations and may shed light on other aspects of dynamical matching markets. In particular, it answers the question: how much one should like someone to propose marriage.



%An important point to emphasize is that the benefits of marriage disclosed with this model concerns just a limited aspect of the matching of couples in large societies. There are many other aspects of our preferences concerning relationships that are not taken into account in this model. These other aspects may obliterate the benefits of marriage we will present. However, the benefits are still robust and seem to hold up under a variety of circumstances, as we will show in the next section.

%Another famous problem related to the one we are interested in here is the secretary problem (see \cite{10.2307/2245639} for a review). Very briefly, the secretary problem is an optimal stopping problem, where applicants for a position are presented sequentially, and it is necessary to choose the best candidate constrained by the condition that an applicant cannot be chosen after being rejected. In its simplest form, the parameter that characterizes applicants is sampled by the same distribution. This feature, however, is one of the essential differences between the secretary problem and the problem we are considering. The marriage market has externalitie, which are manifested in the fact that the probability distributions that samples potential mates changes over time. In principle, with these distributions, which we will compute in section (\ref{analytical}), one could formulate our model as a generalization of the secretary problem.
...
\subsection{Stochastic matching model}
...
\subsection{Marriage model and results}
\label{benefitsofmarriage}

We can implement marriage in our stochastic matching model in the following way. First, we introduce the \textit{proposal vector} $\vec{\Lambda}\in\mathbb{R}^N$, where $\Lambda_k$ encodes how much agent $k$ needs to like another agent to propose marriage. Then, if the affinity between a couple is higher than their respective proposal values, the couple will not match with new agents in the next steps of the evolution. In this case, we say that the couple is married. More precisely, 
...
\begin{figure}
\centering\includegraphics{legend.pdf}
...
%\begin{figure}[H]\begin{subfigure}{.48\textwidth}  \centering\includegraphics[width=\linewidth]{cool.pdf} \caption{More liked}\label{moreliked}\end{subfigure}%\hspace{5mm}\begin{subfigure}{.48\textwidth}\centering \includegraphics[width=\linewidth]{bad.pdf}\caption{Less liked}\label{lessliked}\end{subfigure}\caption{Evolution of average utility for the  more liked (\ref{moreliked}) and less liked (\ref{lessliked}) agents for the Gaussian model $\mathcal{N}(0,1)$ with $N=10.000$, in the model with marriage $\Lambda=1$ (orange) and without it (blue).}\end{figure}
...
%We should add that our model gives no explanation for the shift to higher $\Lambda$, this is a topic of research in many fields.  
